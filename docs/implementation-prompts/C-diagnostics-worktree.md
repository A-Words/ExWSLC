...
请在本任务已经选定的独立工作树内实现 ExWSLC 的 C 组：环境与会话诊断、容器访问本地服务诊断、空闲刷新调度。执行顺序为 08 → 09 → 10。完成本组全部任务后再交付；每完成一项先验证并独立提交，再继续下一项。
...
- [ 08-runtime-diagnostics-sessions.md ](./08-runtime-diagnostics-sessions.md)
- [ 09-host-loopback-diagnostics.md ](./09-host-loopback-diagnostics.md)
- [ 10-idle-aware-refresh.md ](./10-idle-aware-refresh.md)

## 本组边界

C 组共享设置与诊断入口，必须组内串行。先完成 08 的只读诊断数据与展示，再让 09 复用入口，最后做 10 的调度。保留 02 的显式重新检测、安装后的自动刷新恢复与关闭取消边界；不要在普通库存刷新中重复运行能力探测。不要为了诊断或节能而修改原生 WSLC 设置、停止会话或取消用户任务。
...
- 最终返回：组别 C、工作树绝对路径、分支、起始基线 SHA、各任务提交 SHA、当前 HEAD、验证文档／截图、共享接口增量与尚未验证项。
