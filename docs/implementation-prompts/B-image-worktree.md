...
请在本任务已经选定的独立工作树内实现 ExWSLC 的 B 组：高级镜像构建参数、secret、输出产物与真实进度。执行顺序为 06。完成本组全部任务后再交付；每完成一项先验证并独立提交，再继续下一项。
...
- [ 06-advanced-image-build.md ](./06-advanced-image-build.md)

## 本组边界

聚焦镜像工作区。复用 BuildSecret／BuildOutput／BuildProgress／BuildPull 的独立能力项；保留默认简单构建。不能把仅导出目录／tar 的成功误报为已创建本地镜像，secret 值不得进入 build-arg、日志、任务标题或设置。
...
- 最终返回：组别 B、工作树绝对路径、分支、起始基线 SHA、各任务提交 SHA、当前 HEAD、验证文档／截图、共享接口增量与尚未验证项。
